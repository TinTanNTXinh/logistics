...
export class DriverComponent implements OnInit
    ...
    updateOne(): void {
        ...
        this.httpClientService.put(this.prefix_url, {"driver": this.driver}).subscribe(
            (success: any) => {
                // Upload File
                this.uploadFile(this.driver.id);
    ...
    public clearFiles(): void {
        this.files_of_driver = [];
        this.file_view_child.nativeElement.value = '';
    }

    public loadFiles(one_id: number): void {
        this.files_of_driver = this.files.filter(o => o.table_name == 'drivers' && o.table_id == one_id);

        if(this.files_of_driver.length > 0)
            this.download_url = this.fileHelperService.createDownloadUrl(this.files_of_driver[0].id);
    }
