...
@Component({
    selector: 'app-formula-sample',
    templateUrl: './formula-sample.component.html',
    styles: []
})
export class FormulaSampleComponent implements OnInit
    , ICommon, ICrud, IDatePicker, ISearch {

    /** ===== MY VARIABLES ===== **/
    public formula_samples: any[] = [];
    public formula_sample: any;
    ...
    ngOnInit(): void {
        this.title = 'Công thức mẫu';
        this.prefix_url = 'formula-samples';
    ...
    loadOne(id: number): void {
        this.formula_sample = this.formula_samples.find(o => o.id == id);
    }
    ...
    addOne(): void {
        ...
        this.httpClientService.post(this.prefix_url, {"formula_sample": this.formula_sample}).subscribe(
            (success: any) => {
                this.reloadData(success);
                this.clearOne();
    ...
    updateOne(): void {
        ...
        this.httpClientService.put(this.prefix_url, {"formula_sample": this.formula_sample}).subscribe(
            (success: any) => {
                this.reloadData(success);
                this.clearOne();
