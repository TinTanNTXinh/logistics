...
export class TransportComponent implements OnInit
    ...
    addOne(): void {
        ...
        this.setReceivePlaceDeliveryPlace();
    ...
    validateOne(): boolean {
        ...
        if (this.transport.product_id == 0) {
            flag = false;
            this.toastrHelperService.showToastr('warning', `Hàng không được để trống!`);
        }
        if (this.transport.customer_id == 0) {
            flag = false;
            this.toastrHelperService.showToastr('warning', `Khách hàng không được để trống!`);
        }
        if (this.transport.postage_id == 0) {
            flag = false;
            this.toastrHelperService.showToastr('warning', `Cước phí không được để trống!`);
        }
        return flag;
    }
    ...
    private addFormula(rule: string, name: string, value1: string|number, value2: string|number): void {
    ...
    public findPostage(): void {
        ...
        this.httpClientService.get(`${this.prefix_url}/find-postage?query=${JSON.stringify(formulas)}`).subscribe(
            (success: any) => {
                ...
                this.transport.unit_id = success.postage.unit_id;
    ...
    public computeRevenue(): void {
        let total_cost_in_transport: number = (this.transport.carrying + this.transport.parking + this.transport.fine + this.transport.phi_tang_bo + this.transport.add_score);
        if (this.transport.unit_id == 1) {
            this.transport.revenue = Number(this.transport.postage_unit_price) + total_cost_in_transport;
        } else {
            this.transport.revenue = Number(this.transport.quantum_product * this.transport.postage_unit_price) + total_cost_in_transport;
        }
    }
    ...
    private findFormulas(find_formulas: {}): void {
        this.httpClientService.get(`${this.prefix_url}/find-formulas?query=${JSON.stringify(find_formulas)}`).subscribe(
            (success: any) => {
                ...
                // Set Fuel_Id for Transport
                this.transport.fuel_id = this.oil.id;
                ...
                for (let formula of formulas) {
                    switch (formula.rule) {
                        ...
                        case 'RANGE':
                            ...
                            break;
                        case 'OIL':
                            this.addFormula(formula.rule, formula.name, this.oil.price, 0);
                            break;
                        case 'PAIR':
                            this.addFormula(formula.rule, formula.name, '', '');
                            break;
                ...
                // Display ReceivePlace, DeliveryPlace
                this.isRoute = formulas.filter(o => o.rule == 'PAIR' && o.name == 'Tuyến đường').length > 0;
            },
            (error: any) => {
                this.toastrHelperService.showToastr('error');
            }
    ...
    private setReceivePlaceDeliveryPlace(): void {
        let formulas = this.formulaFormArray.value;
        let found = formulas.find(o => o.rule == 'PAIR' && o.name == 'Tuyến đường');
        if(found) {
            this.transport.receive_place = found.value1;
            this.transport.delivery_place = found.value2;
        }
    }
}